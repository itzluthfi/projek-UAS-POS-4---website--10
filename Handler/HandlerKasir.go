...
func GetItemDetailsHandler(w http.ResponseWriter, r *http.Request) {
	...
	message, item := Controller.ValidasiSearchItem(idItem)
	if item == nil {
	...
	response := struct {
		...
		ID:          item.Item.Id,
		Nama:        item.Item.Nama,
		JmlStock:    item.Item.JmlStock,
		Harga:       item.Item.Harga,
		Diskon:      item.Item.Diskon,
		HargaDiskon: item.Item.HargaDiskon,
	}
...
func TambahItemStockHandler(w http.ResponseWriter, r *http.Request) {
	...
	message, success := Controller.ValidasiTambahStokItem(idItem, jumlah)
	if !success {
		http.Error(w, message, http.StatusBadRequest)
		return
	}
...
func KurangiItemStockHandler(w http.ResponseWriter, r *http.Request) {
	...
	message, success := Controller.ValidasiKurangiStokItem(idItem, jumlah)
	if !success {
		http.Error(w, message, http.StatusBadRequest)
		return
	}
